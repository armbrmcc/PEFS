...
class Result extends MainController
{
	...
	* @Create  Date 2565-01-25
    */
    public function show_result_evaluation_type1($group_id, $assessor_id, $nominee_id, $promote, $emp)
    {
        $this->load->model('M_pef_group', 'pef');
        ...
        $data['obj_assessor'] = $this->assessor->get_assessor_by_id($assessor_id)->result();
        $data['obj_group_ass'] = $this->assessor->get_assessor_detail($group_id)->result();
        $data['obj_nominee'] = $this->nominee->get_nominee_by_id($nominee_id)->result();
        $data['arr_nominee'] = $this->nominee->get_nominee_detail($group_id)->result();
        $data['arr_item'] = $this->item->get_item_evaluation_by_id()->result();

        $data['obj_promote'] = $this->nominee->get_promote_to($nominee_id)->result();
        ...
        // print_r($id_assessor);
        // print_r($nor_id);
        $data['arr_per'] = $this->per->get_performance($emp, $assessor_id, $date)->result();
    ...
    */
    public function show_result_evaluation_type2_1($group_id, $id_assessor, $id_nominee, $promote, $nor_id)
    {
        ...
        $this->load->model('M_pef_group_nominee', 'nominee');
        $this->load->model('M_pef_format_form', 'form');
        $this->load->model('M_pef_description_form', 'description');
        $this->load->model('M_pef_item_form', 'item');
        $this->load->model('M_pef_performance_form', 'performance');
        $this->load->model('M_pef_point_form', 'point');
        ...
        $data['arr_item'] = $this->item->get_item_evaluation_by_id()->result();
        $data['arr_des'] = $this->description->get_description_evaluation_by_id()->result();
        $data['arr_nominee'] = $this->nominee->get_nominee_detail($group_id)->result();
        ...
        $data['arr_per'] = $this->performance->get_performance($nor_id, $id_assessor, $date)->result();
        // print_r($data['arr_per']);
        $data['arr_per_id'] = $this->performance->get_performance_by_id($nor_id, $id_assessor, $date)->result();
        $per_get =  $data['arr_per'][0]->per_id;
        // print_r($per_get);
        $data['arr_point_round1'] = $this->point->get_point_list_round1($per_get)->result();
        $data['arr_per'] = $this->performance->get_performance($nor_id, $id_assessor, $date)->result();
        // print_r($data['arr_point_round1']);

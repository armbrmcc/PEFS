...
class Da_pef_performance_form extends pefs_model
{
    ...
    */
    ...
    /*
    * delete_performance_form()
    * ลบข้อมูลใน database ตาราง pef_performance_form
    * @input  per_date, per_emp_id, per_ase_id
    * @output -
    * @author Phatchara Khongthandee and Pontakon Mujit 
    * @Create Date 2565-03-09
    * @Update Date 2565-03-10
    */
    function delete_performance_form()
    {
        $sql = "DELETE FROM pefs_database.pef_performance_form 
                WHERE   per_id = ?";

        $this->db->query($sql, array($this->per_id));
    }//ลบข้อมูล Q&A, Comment, Date(วันที่ประเมิน), รหัส Nominee, Assessor ที่ประเมิน
